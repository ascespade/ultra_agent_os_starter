...
class RateLimitOrchestrator {
  constructor(redisClient, limitsConfig) {
    this.redis = redisClient;
    this.policies = new Map();
    this.circuitBreaker = new CircuitBreaker(5, 30);
    this.circuitBreaker.initialize(redisClient);
    this.limitsConfig = limitsConfig || { ai: { rate: 8, burst: 25 }, light: { rate: 50, burst: 100 } };
    this.setupPolicies();
  }

  setupPolicies() {
    const ai = this.limitsConfig.ai || { rate: 8, burst: 25 };
    const light = this.limitsConfig.light || { rate: 50, burst: 100 };
    this.policies.set('ai_endpoints', {
      bucket: new TokenBucket(ai.rate, ai.burst, this.redis),
      queueEnabled: true,
      timeoutMs: 60000
    });
    this.policies.set('light_endpoints', {
      bucket: new TokenBucket(light.rate, light.burst, this.redis),
      queueEnabled: false,
      timeoutMs: 30000
    });
