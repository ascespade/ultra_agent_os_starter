...
class Dashboard {
      ...
      files.forEach(file => {
        html += `
          <div class="memory-item">
            <span>${file}</span>
            <button class="btn btn-secondary btn-sm" onclick="window.dashboard.loadMemoryFile('${file}')">
              Load
            </button>
          </div>
        `;
  ...
  async loadMemoryFile(filename) {
    try {
      ...
      document.getElementById('memoryContent').value = JSON.stringify(memory.data, null, 2);
    } catch (error) {
