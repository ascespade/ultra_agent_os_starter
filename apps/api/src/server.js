...
} = require("../../../lib/production-env-validator");
...
// ===== UI STATIC FILE SERVING (PRODUCTION MODE) =====
// Serve UI as static assets from API server to eliminate separate UI server
const UI_ENABLED = process.env.UI_ENABLED !== "false";
const UI_PATH = process.env.UI_PATH || "/ui";

if (UI_ENABLED) {
  const uiStaticPath = path.join(__dirname, "../../ui/src");

  // Dynamic env.js for UI (injects API_URL and WebSocket config)
  app.get(`${UI_PATH}/env.js`, (req, res) => {
    res.type("application/javascript");
    const API_URL =
      process.env.RAILWAY_PUBLIC_URL ||
      `http://localhost:${process.env.PORT || 3000}`;
    // WebSocket now on same port as HTTP server
    res.send(
      `window.ENV = { API_URL: '${API_URL}', WS_PORT: '${process.env.PORT || 3000}' };`,
    );
  });

  // UI routes (before static middleware to handle routing)
  app.get(`${UI_PATH}/`, (req, res) => {
    if (req.query.admin === "true") {
      res.sendFile(path.join(uiStaticPath, "index_admin.html"));
    } else {
      res.sendFile(path.join(uiStaticPath, "index.html"));
    }
  });

  app.get(`${UI_PATH}/settings`, (req, res) => {
    res.sendFile(path.join(uiStaticPath, "settings.html"));
  });

  app.get(`${UI_PATH}/test-api`, (req, res) => {
    res.sendFile(path.join(uiStaticPath, "test-api.html"));
  });

  // Serve all other static files (CSS, JS, images)
  app.use(UI_PATH, express.static(uiStaticPath));

  console.log(`[UI] UI serving enabled at ${UI_PATH}`);
}

// Root landing page (fix localhost:3000/)
const UI_URL =
  process.env.UI_URL ||
  process.env.API_URL?.replace(":3000", ":8088") ||
  "http://localhost:8088";
app.get("/", (req, res) => {
  res.type("html");
  res.send(`
...
// JWT Secret with strict production guard
// SECURITY: JWT_SECRET must be set explicitly in production
// Use scripts/setup-env.js for local development
const JWT_SECRET = process.env.JWT_SECRET;

// Runtime security check - fail fast if missing
if (!JWT_SECRET) {
  console.error('[SECURITY] CRITICAL: JWT_SECRET environment variable is required');
  console.error('[SECURITY] For local dev: Run "node scripts/setup-env.js" to generate .env.local');
  console.error('[SECURITY] For production: Set JWT_SECRET to a secure random string (64+ chars)');
...
if (JWT_SECRET.length < 32) {
  ...
  console.error('[SECURITY] Current length:', JWT_SECRET.length);
  process.exit(1);
}
...
// This eliminates the need for a separate WebSocket port
let wss = null;

function broadcastLog(jobId, logEntry) {
  if (!wss) return; // WebSocket not initialized yet
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
      ...
      if (envPassword && envPassword.length >= 8) {
        ...
        console.log(
          "[SECURITY] Admin password synced to DEFAULT_ADMIN_PASSWORD",
        );
      } else {
    ...
    if (!envPassword || envPassword.length < 8) {
      ...
      console.log(
        "[SECURITY] Set DEFAULT_ADMIN_PASSWORD for predictable credentials.",
      );
    }
  } catch (error) {
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (username.length < 1 || username.length > 50) {
    return res
      .status(400)
      .json({ error: "Username must be between 1 and 50 characters" });
  }

  if (password.length < 1 || password.length > 100) {
    return res
      .status(400)
      .json({ error: "Password must be between 1 and 100 characters" });
  }
  ...
  try {
    ...
    const status = {
      ...
      redis: {
        ...
        backlog_limit: 100, // Sync with MAX_BACKLOG in POST /api/chat
...
app.post("/api/memory/:filename", withTenant, async (req, res) => {
  ...
  console.log(
    `[MEMORY] POST received - filename: ${filename}, data type: ${typeof data}`,
  );
  ...
  try {
    ...
    console.log(
      `[MEMORY] Storing - userId: ${userId} (type: ${typeof userId}), tenant: ${req.tenantId}, file: ${filename}`,
    );

    // Phase 6: JSONB Fix - Pass object directly, pg handles stringification
    ...
    const result = await db.query(
      ...
      RETURNING id
    `,
      [userId, filename, contentToStore, req.tenantId],
    );

    console.log(
      `[MEMORY] Query executed, rows returned: ${result.rows.length}`,
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to store memory", details: error.message });
  }
});
...
serverBootstrap()
  .then(() => {
    ...
    console.log(
      `[ENV] Profile: ${profile.env} (logging=${profile.logging}, limits=${profile.limits})`,
    );
    console.log(
      "[RATE_LIMIT] Initializing professional rate limiting system...",
    );
    const rateLimitOrchestrator = new RateLimitOrchestrator(
      redisClient,
      profile.limitsConfig,
    );

    // Rate limiting with tenant-prefixed keys (ISOLATION_RULES.redis)
    const tenantRateKey = (req) =>
      `tenant:${(req.headers["x-tenant-id"] || "none").trim()}:${req.user?.userId || req.ip}`;
    app.use(
      "/api/chat",
      rateLimitOrchestrator.getMiddleware("ai_endpoints", tenantRateKey),
    );
    app.use(
      "/api/jobs",
      rateLimitOrchestrator.getMiddleware("light_endpoints", tenantRateKey),
    );
    app.use(
      "/api/memory",
      rateLimitOrchestrator.getMiddleware("light_endpoints", tenantRateKey),
    ...
    app.use(
      "/api/auth",
      rateLimitOrchestrator.getMiddleware("light_endpoints", (req) => req.ip),
    );
    ...
    app.get("/api/plugins", withTenant, async (req, res) => {
      try {
        ...
        const enabled = await pluginSystem.getEnabledPlugins(
          redisClient,
          req.tenantId,
        );
        const list = (discovered.available || []).map((p) => {
          const validation = pluginSystem.validatePlugin(
            p,
            pluginSystem.getPluginDir(),
            { signedOnly: true },
          );
    ...
    app.post("/api/plugins/:pluginId/enable", withTenant, async (req, res) => {
      ...
      try {
        const discovered = pluginSystem.discoverPlugins();
        const meta = (discovered.available || []).find(
          (p) => p.id === pluginId,
        );
        if (!meta) {
        ...
        const result = await pluginSystem.enablePlugin(
          redisClient,
          tenantId,
          pluginId,
          meta,
        ...
        if (!result.success) {
          return res
            .status(400)
            .json({ error: result.error || "Enable failed" });
        }
    ...
    app.post("/api/plugins/:pluginId/disable", withTenant, async (req, res) => {
      const { pluginId } = req.params;
      try {
        const result = await pluginSystem.disablePlugin(
          redisClient,
          req.tenantId,
          pluginId,
        );
    ...
    app.get("/api/admin/rate-limit-metrics", withTenant, async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to retrieve rate limit metrics" });
      }
    });
    ...
    app.get("/api/user/settings", withTenant, async (req, res) => {
      try {
        ...
        if (result.rows.length > 0) {
          ...
          const decryptedSettings = security.decryptSettings(
    ...
    app.post("/api/user/settings", withTenant, async (req, res) => {
      try {
        ...
        const oldSettings =
          oldResult.rows.length > 0 ? oldResult.rows[0].settings : null;
    ...
    app.get("/api/health/detailed", withTenant, async (req, res) => {
      try {
        ...
        `,
          ...
          jobMetrics.total = jobResult.rows.reduce(
    ...
    initializeDefaultUser()
      .then(async () => {
        // PRODUCTION FIX: Single port binding (no port sprawl)
        // In production (Railway), use $PORT. In dev, use 3000 or dynamic allocation.
        const isProduction = process.env.NODE_ENV === "production";
        const isRailway = !!process.env.RAILWAY_ENVIRONMENT;

        let PORT;
        if (isProduction || isRailway) {
          // Production: Use $PORT from env (Railway provides this)
          PORT = parseInt(process.env.PORT || "3000", 10);
          console.log(
            `[PRODUCTION] Using fixed PORT=${PORT} (no dynamic allocation)`,
          );
        } else {
          // Development: Allow dynamic port allocation
          const preferredPort = process.env.PORT
            ? parseInt(process.env.PORT)
            : 3000;
          PORT = await getAvailablePort("api", preferredPort);
          console.log(`[DEVELOPMENT] Using dynamic PORT=${PORT}`);
        }

        // Listen on 0.0.0.0 in production, localhost in dev
        const HOST =
          process.env.HOST ||
          (isProduction || isRailway ? "0.0.0.0" : "127.0.0.1");

        const server = app.listen(PORT, HOST, () => {
          console.log(
            `[CORE] Ultra Agent API running on ${HOST}:${PORT} (env=${profile.env})`,
          );
            ...
            ws.on("message", async (message) => {
              try {
                ...
                if (data.type === "subscribe_logs") {
                  ws.jobId = data.jobId;
                  console.log(
                    `[WEBSOCKET] Client subscribed to job ${data.jobId}`,
                  );
                }
              } catch (error) {
          ...
          console.log(
            `[WEBSOCKET] WebSocket server attached to HTTP server on port ${PORT}`,
          );
          console.log(
            `[PRODUCTION] Single port binding: ${PORT} (HTTP + WebSocket)`,
          );
        });
