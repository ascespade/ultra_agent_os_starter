...
// In Railway, clear API port cache and use Railway's assigned port
if (isRailway && process.env.PORT) {
  const railwayPort = parseInt(process.env.PORT);
  console.log(`[PORT_ALLOC] Railway detected, using assigned PORT=${railwayPort}`);
  
  // Clear any cached API port to force use of Railway port
  try {
    const cache = loadPortCache();
    if (cache.api && cache.api !== railwayPort) {
      console.log(`[PORT_ALLOC] Clearing cached API port ${cache.api}, using Railway port ${railwayPort}`);
      delete cache.api;
      savePortCache(cache);
    }
  } catch (error) {
  ...
  // Ensure Railway port is in our acceptable range
