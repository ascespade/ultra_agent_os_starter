...
// In Railway, immediately clear/set API port cache to force Railway port usage
if (isRailway && railwayPort) {
  ...
  try {
    ...
    // Force API port to Railway port
    cache.api = railwayPort;
    fs.writeFileSync(cacheFile, JSON.stringify(cache, null, 2));
    console.log(`[PORT_ALLOC] Set API cache to Railway port ${railwayPort}`);
  } catch (error) {
    console.log(`[PORT_ALLOC] Creating new cache with Railway port ${railwayPort}: ${error.message}`);
    // Create new cache with Railway port
...
async function getAvailablePort(serviceName, preferredPort = null) {
  ...
  // In Railway, always use Railway's assigned PORT (proxy forwards to it)
  if (isRailway && process.env.PORT) {
    const railwayPort = parseInt(process.env.PORT);
    console.log(`[PORT_ALLOC] Railway ${serviceName}: using assigned PORT=${railwayPort}`);
    
    cache[serviceName] = railwayPort;
    savePortCache(cache);
    return railwayPort;
  }
...
async function initializeAllPorts() {
  ...
  try {
    // In Railway, use Railway's assigned PORT for API service
    const apiPreferredPort = (isRailway && railwayPort) ? railwayPort : 3001;
    
    ports.api = await getAvailablePort('api', apiPreferredPort);
    ports.ui = await getAvailablePort('ui', 3100);
    ports.worker = await getAvailablePort('worker', 3200);
    ports.websocket = await getAvailablePort('websocket', 3300);
