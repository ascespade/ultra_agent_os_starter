...
// Use environment variable if available, otherwise fallback (will fail if mismatched)
const PASSWORD = process.env.DEFAULT_ADMIN_PASSWORD || 'admin123';

console.log('='.repeat(60));
console.log('FULL SYSTEM VALIDATION (CONTAINER INTERNAL)');
console.log('='.repeat(60));
  ...
  try {
    ...
    try {
      ...
      if (redis.backlog_limit === 100) {
        ...
        console.error(`  ✗ Backlog limit missing/incorrect: ${redis.backlog_limit}`);
      }
    } catch (error) {
    ...
    try {
      // Store
      try {
         await client.post(`/api/memory/${memoryFile}`, { data: memoryData });
         console.log(`  ✓ Memory stored: ${memoryFile}`);
      } catch (e) {
         console.log('  -> Store Failed Details:', JSON.stringify(e.response?.data || e.message));
         throw e;
      }

      // Retrieve
      const retrieved = await client.get(`/api/memory/${memoryFile}`);
      // Check deep equality manually or just check known property
      if (retrieved.data.content && retrieved.data.content.complex && retrieved.data.content.complex.nested[1] === 2) {
         console.log(`  ✓ Memory retrieved and verified details`);
      } else if (retrieved.data.complex && retrieved.data.complex.nested[1] === 2) {
         console.log(`  ✓ Memory retrieved and verified (direct object)`);
      } else {
         console.log('Memory content:', JSON.stringify(retrieved.data, null, 2));
    ...
    // UI is served from /ui/ path by express
    try {
